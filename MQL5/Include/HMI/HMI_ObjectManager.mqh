...
void OM_Rect(const string name, const datetime t1, const double p1,
             ...
             const bool fill, const ENUM_LINE_STYLE style)
  {
...
void OM_Text(const string name, const datetime t, const double p,
             const string text, const color c, const ENUM_ANCHOR_POINT anchor)
  {
...
void OM_Level(const string name, const datetime t1, const datetime t2,
              const double price, const color c, const ENUM_LINE_STYLE style)
  {
...
void OM_Panel(const int line, const string text, const color c)
  {
   ...
   ObjectSetInteger(0, name, OBJPROP_YDISTANCE, InpPanelY + line * (InpFontSize + 6));
   ...
   if(InpShowH4Context)
     {
      ...
      OM_Panel(0, "HMI v" + HMI_VERSION + "  MARK ONLY - NO ENTRY DECISION", InpColText);
      OM_Panel(1, s1, InpColText);
      OM_Panel(2, s2, InpColText);
      OM_Panel(3, s3, InpColText);
      OM_Panel(4, s4, InpColText);
     }
   ...
   if(InpShowTradingRange && g_tr_n > 0)
     {
      ...
      OM_Rect(OM_Name(TT_TR, g_tr[i].version_id, 0), g_tr[i].valid_from, g_tr[i].a_hi,
              redge, g_tr[i].a_lo, InpColTRange, false, STYLE_DOT);
     }
      ...
      for(int i = 0; i < g_liq_n; i++)
        {
         ...
         OM_Level(OM_Name(TT_LIQ, g_liq[i].id, 0), g_liq[i].origin_time, redge,
                  g_liq[i].price, InpColTRange, STYLE_DOT);
        }
      ...
      for(int i = 0; i < g_poi_n; i++)
        {
         ...
         color c = dead ? InpColPOIDead : (g_poi[i].dir == DIR_BULL ? InpColPOIBull : InpColPOIBear);
         ...
         OM_Rect(OM_Name(TT_POI, g_poi[i].id, 0), g_poi[i].origin_time, g_poi[i].hi,
                 t2, g_poi[i].lo, c, false, dead ? STYLE_DOT : STYLE_SOLID);
         OM_Text(OM_Name(TT_POI, g_poi[i].id, 1), g_poi[i].origin_time, g_poi[i].hi,
                 "H4 POI", c, ANCHOR_LEFT_LOWER);
         g_poi[i].vis = vis;
        }
      ...
      for(int i = 0; i < g_blk_n; i++)
        {
         ...
         color c = dead ? InpColBlockDead
                        : (g_blk[i].state == BLOCK_ARMED ? InpColBlockArmed
                          : (g_blk[i].dir == DIR_BULL ? InpColBlockBull : InpColBlockBear));
         ...
         OM_Rect(OM_Name(TT_BLK, g_blk[i].id, 0), g_blk[i].origin_time, g_blk[i].hi,
                 t2, g_blk[i].lo, c, false, dead ? STYLE_DOT : STYLE_SOLID);
         g_blk[i].vis = vis;
        }
      ...
      if(InpShowArmedLabel && (cy.drawn_mask & (1 << 6)) == 0)
        {
         ...
         OM_Text(OM_Name(TT_ARM, cy.cycle_id, 0), cy.armed_time, p, txt, InpColBlockArmed,
      ...
      for(int m = 0; m < MDL_COUNT; m++)
        {
         ...
         OM_Text(OM_Name(TT_MDL, cy.cycle_id, m), cy.cfm_time[m], p,
                 ...
                 cy.dir == DIR_BULL ? InpColBlockBull : InpColBlockBear,
                 cy.dir == DIR_BULL ? ANCHOR_UPPER : ANCHOR_LOWER);
         cy.drawn_mask |= (1 << m);
        }
      ...
      for(int i = 0; i < g_bpr_n; i++)
        {
         ...
         color c = (vis == 2 ? InpColBPRDead : InpColBPR);
         ...
         OM_Rect(OM_Name(TT_BPR, g_bpr[i].id, 0), g_bpr[i].formation_time, g_bpr[i].hi,
                 t2, g_bpr[i].lo, c, false, vis == 2 ? STYLE_DOT : STYLE_SOLID);
         g_bpr[i].vis = vis;
        }
  }
   ...
   for(int i = 0; i < g_blk_n; i++)
     {
      ...
      OM_Text(nm, t0, (g_blk[i].dir == DIR_BULL ? g_blk[i].lo - off : g_blk[i].hi + off),
              "PENDING TOUCH", InpColBlockArmed,
              g_blk[i].dir == DIR_BULL ? ANCHOR_UPPER : ANCHOR_LOWER);
      ObjectSetInteger(0, nm, OBJPROP_FONTSIZE, MathMax(6, InpFontSize - 1));
      return;
     }
  }
